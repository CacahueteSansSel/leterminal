...
int16_t extapp_drawTextLarge(const char *text, int16_t x, int16_t y, uint16_t fg, uint16_t bg, bool fake) {
  ...
  for (int i = 0; i < count; i++) {
    Terminal::Screen::drawChar(ctx, text[i], KDColor::RGB16(bg), KDColor::RGB16(fg), point.translatedBy(KDPoint(i * 8, 0)));
  }
...
int extapp_fileListWithExtension(const char ** filenames, int maxrecords, const char * extension, int storage) {
  if(storage == EXTAPP_RAM_FILE_SYSTEM) {
    ...
    return n;
  } else if(storage == EXTAPP_VFS_FILE_SYSTEM) {
    // TODO: filter by extension too
    int n = Terminal::VFS::VirtualFS::sharedVFS()->current()->childCount();
    if (n > maxrecords) {
      n = maxrecords;
    }
    for (int i = 0; i < n; i++) {
...
bool extapp_fileExists(const char * filename, int storage) {
  if(storage == EXTAPP_RAM_FILE_SYSTEM) {
    ...
    return Terminal::VFS::VirtualFS::sharedVFS()->current()->provideChild(filename) != nullptr;
  } else {
    return false;
  }
}
...
bool extapp_fileErase(const char * filename, int storage) {
  if(storage == EXTAPP_RAM_FILE_SYSTEM) {
    ...
    return Terminal::VFS::VirtualFS::sharedVFS()->current()->remove(filename);
  } else {
    return false;
  }
}
...
const char * extapp_fileRead(const char * filename, size_t *len, int storage) {
  if(storage == EXTAPP_RAM_FILE_SYSTEM) {
    ...
    auto child = Terminal::VFS::VirtualFS::sharedVFS()->current()->provideChild(filename);
    if (child == nullptr) return NULL;

    if (len) {
      *len = (size_t)child->dataLength();
    }
    return (const char*)child->data();
  } else {
    return NULL;
  }
}
